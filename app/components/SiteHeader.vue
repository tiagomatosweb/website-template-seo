<template>
  <div :class="overlay ? '' : 'relative'">
    ...
    <div v-if="!overlay" id="top" class="hidden lg:block relative z-51 bg-primary text-white/85 border-b border-white/10">
      <UContainer>
        <div class="flex items-center justify-between gap-6 min-h-11">
          <!-- PLACEHOLDER: tagline -->
          <span class="inline-flex items-center font-display font-semibold text-xs tracking-wide text-white/85">Your trusted local specialists</span>
          <div class="flex items-center gap-3">
            <a href="#" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center size-7 rounded-full text-white/85 transition-colors duration-150 hover:text-white hover:bg-white/12">
    ...
    <nav
      ...
      :class="[
        ...
        overlay && solid ? 'bg-white shadow-md' : '',
      ]"
    >
      <UContainer>
        <div class="flex items-center justify-between gap-8 min-h-22">
          <div class="flex items-center gap-10">
            <NuxtLink to="/" class="flex items-center">
              ...
              <span class="font-display font-black text-2xl tracking-tight" :class="light ? 'text-white' : 'text-neutral-900'">Business<span :class="light ? 'text-primary-300' : 'text-primary'">Name</span></span>
            </NuxtLink>
            <div class="flex items-center gap-8">
              <NuxtLink
                ...
                :class="light ? 'text-white/90 hover:text-white' : 'text-neutral-600 hover:text-primary'"
              >{{ link.label }}</NuxtLink>
            </div>
          </div>
          <div class="flex items-center gap-5">
            <!-- PLACEHOLDER: phone number -->
            <a href="tel:0000000000" class="group flex flex-col items-end leading-none text-right py-0.5">
              <span class="font-mono text-xs tracking-widest uppercase mb-1" :class="light ? 'text-white/60' : 'text-neutral-900/50'">Call Now</span>
              <span class="font-display font-extrabold text-2xl tracking-normal tabular-nums transition-colors duration-150" :class="light ? 'text-white group-hover:text-primary-300' : 'text-neutral-900 group-hover:text-primary'">00 0000 0000</span>
            </a>
      ...
      <UContainer>
        <div class="flex items-center justify-between h-16">
          <NuxtLink to="/" class="flex items-center shrink-0">
            <!-- PLACEHOLDER: replace wordmark with logo image -->
            <span class="font-display font-black text-xl tracking-tight" :class="light ? 'text-white' : 'text-neutral-900'">Business<span :class="light ? 'text-primary-300' : 'text-primary'">Name</span></span>
          </NuxtLink>

          <div class="flex items-center gap-3">
            <!-- PLACEHOLDER: phone number -->
            <a href="tel:0000000000" class="inline-flex items-center gap-1.5 font-display font-extrabold text-base tabular-nums transition-colors duration-150" :class="light ? 'text-white' : 'text-neutral-900 hover:text-primary'">

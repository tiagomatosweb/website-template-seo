<template>
  <UPageSection>
    <div class="relative overflow-hidden rounded-2xl bg-primary-950 shadow-[0_30px_80px_-20px] shadow-primary-950/50 ring-1 ring-white/10">
        ...
        <div class="relative grid sm:grid-cols-3">
          <div class="relative flex flex-col items-center px-6 py-12 text-center">
            ...
            <div class="mt-2 inline-flex items-center gap-2 rounded-full bg-white/5 px-3 py-1 ring-1 ring-white/10 backdrop-blur">
            ...
            <div class="mt-2 text-sm text-white/45">Rated by our customers</div>
          </div>
          ...
          >
            ...
            <div v-if="stat.caption" class="mt-0.5 text-sm text-white/45">{{ stat.caption }}</div>
          </div>
        </div>

        <div class="relative flex flex-wrap items-center justify-center gap-x-8 gap-y-2 border-t border-white/10 bg-linear-to-r from-transparent via-white/4 to-transparent px-6 py-5">
          <span
            v-for="pill in props.pills"
            :key="pill"
            class="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-white/80"
          >
...
const props = withDefaults(defineProps<{
  ...
  pills: () => ['Family owned & operated', 'Fully licensed & insured'],
})
</script>

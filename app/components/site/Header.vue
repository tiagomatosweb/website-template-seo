        ...
        <div class="flex items-center justify-between gap-8 min-h-22">
          <div class="flex items-center gap-10">
            <NuxtLink to="/" class="flex items-center">
              ...
              <span class="font-display font-black text-2xl tracking-tight" :class="light ? 'text-inverted' : 'text-highlighted'">{{ site.name }}</span>
            </NuxtLink>
            <div class="flex items-center gap-8">
              <NuxtLink
                ...
                :class="light ? 'text-toned-inverted hover:text-inverted' : 'text-toned hover:text-primary'"
              >{{ link.label }}</NuxtLink>
            </div>
          </div>
      ...
      <UContainer>
        <div class="flex items-center justify-between h-16">
          <NuxtLink to="/" class="flex items-center shrink-0">
            <!-- PLACEHOLDER: replace wordmark with logo image -->
            <span class="font-display font-black text-xl tracking-tight" :class="light ? 'text-inverted' : 'text-highlighted'">{{ site.name }}</span>
          </NuxtLink>
          ...
          <div class="flex items-center gap-3">
            ...
            <USlideover v-model:open="menuOpen" title="Menu" side="right">
              <UButton
                ...
                :class="light ? 'text-inverted hover:bg-surface-soft-inverted' : ''"
              />
              ...
              <template #body>
                <nav class="flex flex-col">
                  <NuxtLink
                    ...
                    :key="link.href"
                    :to="link.href"
                    class="block py-3 text-base font-display font-bold tracking-widest uppercase text-highlighted border-b border-default transition-colors duration-150 hover:text-primary"
